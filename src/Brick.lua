...
function Brick:init(x, y, powerUp)
    ...
    -- used to determine wheter the brick is locked or not
    self.locked = false
...
function Brick:hit()
    ...
    -- over the particle's lifetime (the second color)
        ...
        if self.tier > 0 then
            ...
            if self.color == 1 then
                -- if powerUp exists for brick enable it
...
function Brick:render()
    if self.inPlay then
        ...
        -- if destroyed render the power up if exists
        if self.powerUp ~= nil then
            self.powerUp:render()
        end
    end
