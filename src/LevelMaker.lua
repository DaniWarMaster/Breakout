...
function LevelMaker.createMap(level)
    ...
    for y = 1, numRows do
        ...
        for x = 1, numCols do
            ...
            local generatePowerUp = math.random(0, 100)
            if generatePowerUp < 30 then
                b.powerUp = PowerUp(
                        b.x,
                        b.y,
                        1
                    )
            elseif generatePowerUp < 37 then
                b.powerUp = PowerUp(
                        b.x,
                        b.y,
                        10
            ...
            -- generate locks
            local generateLock = math.random(0, 100)
            if generateLock < 10 then
                b.locked = true
            end
    ...
    -- inserting a key into an unlocked brick
    if #bricks > 0 then
        local keyNotInserted = true
        local tries = 0
        while keyNotInserted and tries < 25 do
            local brickK = math.random(1, #bricks)
            if bricks[brickK].locked == false then
                bricks[brickK].powerUp = PowerUp(
                        bricks[brickK].x,
                        bricks[brickK].y,
                        10
                    )
